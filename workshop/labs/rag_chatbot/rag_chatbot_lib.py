...
def process_tool(response_message, messages, bedrock, tool_list):
    ...
    for content_block in response_content_blocks:
        if 'toolUse' in content_block:
            ...
            if tool_use_block['name'] == 'get_amazon_bedrock_information':
                ...
                collection = get_collection("../../data/chroma_new", "bedrock_faqs_collection")
    ...
    if len(follow_up_content_blocks) > 0:
        ...
        if 'text' in content:
            return True, content['text'] #tool used, response
        else:
            # If no text key, return the whole content or a default message
            return True, str(content) #tool used, response
